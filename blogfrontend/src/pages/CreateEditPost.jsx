...
function CreateEditPostPage({ setCurrentPage, editPost = null }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // Handle post creation/update logic
        ...
        <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-2xl shadow-xl p-8`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className={`border-2 border-dashed ${isDark ? 'border-gray-600' : 'border-gray-300'} rounded-lg p-6 text-center`}>
                {imagePreview ? (
                  <div className="relative">
                    ...
                    <button type="button" onClick={() => { setImagePreview(''); setImage(''); }} className="absolute top-2 right-2 p-2 bg-red-500 text-white rounded-full hover:bg-red-600">
                      <X className="w-5 h-5" />
                    </button>
                  </div>
                ) : (
                  <div>
                    ...
                    <p className={`mb-2 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>Click to upload or drag and drop</p>
                    <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>PNG, JPG or GIF (max 5MB)</p>
                  </div>
                )}
                <input type="file" accept="image/*" onChange={handleImageChange} className="hidden" id="image-upload" />
                <label htmlFor="image-upload" className="mt-4 inline-block px-6 py-2 bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-lg cursor-pointer hover:opacity-90 transition">
                  Choose File
                </label>
              </div>
            </div>
            ...
            <div className="flex space-x-4">
              <button type="submit" className="flex-1 px-6 py-3 bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-lg font-semibold hover:opacity-90 transition">
                {editPost ? 'Update Post' : 'Publish Post'}
              </button>
              <button type="button" onClick={() => setCurrentPage('home')} className={`px-6 py-3 rounded-lg ${isDark ? 'bg-gray-700 text-white hover:bg-gray-600' : 'bg-gray-200 text-gray-900 hover:bg-gray-300'}`}>
                Cancel
              </button>
            </div>
          </form>
